...
// drawn card sits in the ghost cell; accepting it re-melds the eleven; undoing puts the ten back in
// their boxes with the ghost cell open again. The layouts differ by design (six columns in two rows
...
Object.entries(VIEWPORTS).forEach(([name, vp]) => {
  test.describe(name, () => {
    ...
    test('a stock draw moves nothing until the card is accepted; undo puts it back', async ({
      player,
      project,
    }) => {
      const { page } = player;
      ...
      // Accept: eleven cards in eleven slots, the dot still on the drawn card, no ghost cell.
      ...
      // Undo: ten cards back in their boxes, the ghost cell open, the stock tappable again.
