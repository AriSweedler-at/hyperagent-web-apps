// A finished pass-and-play Briscola match is recorded (the owner, 2026-09-25: "after a game is
...
// or losses"): a position seated through `window.__briscola.setup` at one game from the match,
// three tricks, and the documented hook's `recentGames()` (web/shared/edge/boot.ts) lists one
// record with every name, the games won per side and the winning side; the history sheet shows
// the line under the match's events (web/shared/ui/recentGames.ts); a reload keeps the record
...
test('the game that decides the match records a win for the first player with every name and the games won; the sheet lists it; a reload keeps it', async ({
  player,
  project,
}) => {
  const { page } = player;
  ...
  // The first player holds the bastoni honours against low coppe at 1–0 in a best of three.
  await briscolaSetup(
    page,
    briscolaPosition({
      hands: [
  ...
  // The history sheet (the table's buttons are behind the end screen; the reducer's intent opens
  // it anywhere): the match's events, then the one finished match under them.
  ...
  await expect(line.locator('.recent-game-score')).toHaveText('2–0');
  await expect(line.locator('.recent-game-players')).toHaveText(
    over.players.map((p) => p.name).join(' · '),
  );
  // A reload: the finished match is not resumed, the record is (browser storage), the hook lists it.
  await page.reload();
