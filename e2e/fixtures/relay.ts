// What the relay-forced spec needs (e2e/shell-relay.spec.ts, over every game; fidice through its
// row in e2e/fixtures/online-games.ts): the skip when no TURN relay is at hand, the toast a page
// writes once its channel runs through one (PATH_RELAY_MSG in web/shared/edge/peer.ts, shared by
// gin and backgammon; fidice's describePath in web/games/fidice/src/net/peerjs.ts), and the proof
// from the connection itself that the selected candidate pair goes through the relay.
