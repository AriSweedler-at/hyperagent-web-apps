...
// laptop (the tab bar's fit is e2e/shell-home.spec.ts's, for both shell games): a tap on
...
Object.entries(VIEWPORTS).forEach(([name, vp]) => {
  test.describe(name, () => {
    test('About: a tap on "knock" opens Rules at Knock, flashed; a rule links onward', async ({
      player,
      project,
    }) => {
      const { page } = player;
