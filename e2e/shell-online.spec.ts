// Two peers on both shell games through the local PeerServer (or 0.peerjs.com under
// E2E_BROKER=cloud): the host opens a room, the guest joins by the code read off the host's screen,
// the host sees the join and starts, both tables show the same opening (gin's deal: the same upcard,
// ten cards each, 31 in the stock, hand 1; backgammon's seeded opening roll, the host seat 0 and the
// guest seat 1) with the names crossed over, and the recorded Peer constructions prove the ?peer=
// broker override and the ?ice= configuration reached PeerJS under the game's own peer id
// (web/shared/lib/roomCode.ts `peerIdFor`). The play that follows is each game's own:
// e2e/gin-online.spec.ts (a draw and a discard over the wire, the discards sheet) and
// e2e/backgammon-online.spec.ts (a roll out of turn refused, the guest's roll rolled by the host, a
// move propagating). Both origins: a spec about the origin. Tagged per game (see
// shell-home.spec.ts) and @online (ci.yml's broker job and the nightly grep for it).
