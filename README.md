...
| App                          | Live                                                                  | Source                 |
| ---------------------------- | --------------------------------------------------------------------- | ---------------------- |
| Gin Rummy                    | https://arisweedler-at.github.io/hyperagent-web-apps/games/gin-rummy/ | `web/games/gin-rummy/` |
| Fidice (one-cup liar's dice) | https://arisweedler-at.github.io/hyperagent-web-apps/games/fidice/    | `web/games/fidice/`    |

Both games are served from their TypeScript ports under `web/games/`, built by Vite into `dist/games/<name>/` (an unhashed `index.html`, its `app-[hash].js` beside it, and, under `dist/shared/assets/`, the shared chunk with the shared stylesheet beside it (`web/shared/styles`, step 14) and `<name>-[hash].css`; PeerJS and the ICE loader are bundled, no CDN script): Fidice since `docs/MIGRATION.md` step 7 (the legacy bundle split into ES modules by `tools/legacy/debundle-fidice.ts`, then typed), Gin Rummy since step 13 (ported module by module against sha256-pinned cuts of the legacy page and a DOM-snapshot oracle). The original single-file pages stay in the repo under `legacy/`, unserved and never edited, as the frozen sources of the parity oracles (`legacy/README.md`). Rolling either cutover back is reverting its commit. After such a revert, or any deploy that changes a page's bundle hash, a browser holding the cached `games/<name>/index.html` (both origins send `cache-control: max-age=600`) can get a 404 for the previous `app-[hash].js` or shared chunk and show an empty page for up to 10 minutes until it reloads; the self-contained legacy pages had no such window.
...
npm run test:dist      # guards on dist/: relative asset URLs, paths on both origins, module pages, landing page parity with web/
...
npm run test:e2e       # build dist/, then Playwright: every spec on both emulated origins
...
`npm run test:e2e` (first time: `npx playwright install chromium`) builds `dist/` and runs every spec in `e2e/` on two Playwright projects: `pages` (`dist/` under `/hyperagent-web-apps/` on `tools/serve-dist.ts`, like GitHub Pages) and `proxy` (short URLs on `tools/proxy-dev.ts`, which runs the real `infra/games-proxy/worker.ts` against the pages origin, like games.sweedler.com). The config starts both servers and a local PeerServer (`peer` package, :9000); the online specs open a host and a guest context that meet there through the pages' `?peer=host:port` hook and take a STUN-only ICE list from `e2e/fixtures/e2e-ice.json` through `?ice=`, so no real network is needed. PeerJS and Google Fonts are answered from local copies. Each context gets a seeded `Math.random` (`e2e/browser/seed-random.js`), so deals and dice repeat. `e2e/gin-dom-parity.spec.ts` plays the same pass-and-play game and Score Counter session on the served gin page and on the frozen `legacy/gin-rummy/index.html`, which the pages server publishes under `legacy/games/gin-rummy/` through `--alias` (its `../../shared/ice.js` resolves to the aliased legacy copy), and compares the screens at 84 checkpoints (`tools/parity/gin-dom-parity.ts` runs the same driver by hand). `E2E_BROKER=cloud npm run test:e2e -- --grep @online` plays the online specs through 0.peerjs.com instead; CI runs that as the advisory `broker` job. The HTML report lands in `playwright-report/` (`npx playwright show-report`). A bare `playwright test` needs `npm run build` to have run: `tools/serve-dist.ts` exits at once when its root has no `index.html`.
...
web/games/gin-rummy/         Gin Rummy in TypeScript: index.html (legacy markup), theme.css (legacy CSS), main.ts, src/**; built to dist/games/gin-rummy/
web/games/fidice/            Fidice in TypeScript: index.html, theme.css, main.ts, src/** (de-bundled from the legacy page, then typed); built to dist/games/fidice/
legacy/                      the pre-migration pages and shared/ice.js, verbatim; never served, never edited: frozen oracle sources (legacy/README.md)
vite.config.ts               Vite root web/, base './', input = every web/**/index.html
