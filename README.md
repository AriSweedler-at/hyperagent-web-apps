...
Fidice is served from its port in `web/games/fidice/` (the legacy bundle split into ES modules by `tools/legacy/debundle-fidice.ts`, built by Vite into `dist/games/fidice/`) since `docs/MIGRATION.md` step 7; `legacy/fidice/index.html` stays in the repo, unserved, as the frozen source the oracle fixtures and the de-bundler are cut from. Rolling the cutover back is reverting that one commit (it puts `'fidice'` back in `DEFAULT_LEGACY_PAGES`). After that revert, or any deploy that changes the fidice bundle hash, a browser holding the cached `games/fidice/index.html` (both origins send `cache-control: max-age=600`) can get a 404 for the previous `app-[hash].js` and show an empty page for up to 10 minutes until it reloads; the self-contained legacy page had no such window.
...
npm run test:e2e       # build both trees, then Playwright: every spec on both emulated origins against dist/, the smoke spec on dist-next/
...
web/games/fidice/            Fidice as ES modules: index.html, theme.css, main.js, src/** (generated from the legacy bundle); built to dist/games/fidice/
legacy/gin-rummy/index.html  Gin Rummy, verbatim; copied to dist/games/gin-rummy/ by the passthrough plugin
legacy/fidice/index.html     Fidice, verbatim; not served since step 7, kept as the source of the oracle fixtures and the de-bundle
