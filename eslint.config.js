...
export default defineConfig([
  ...
  // --- De-bundled fidice modules (docs/MIGRATION.md step 6) -----------------------------------
  {
    // Generated by tools/legacy/debundle-fidice.ts: the bodies are the legacy esbuild bundle byte
    // for byte, so the three rules that verbatim code breaks are off here until step 8 types the
    // modules into .ts under the full profile. `no-undef` stays on with exactly the browser
    // globals the page uses: it proves every other free identifier was resolved to an import.
    files: ['web/games/fidice/main.js', 'web/games/fidice/src/**/*.js'],
    languageOptions: {
      globals: {
        window: 'readonly',
        document: 'readonly',
