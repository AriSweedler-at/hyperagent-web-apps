...
| `web/shared/lib/events.ts` | pure | `GameEvent<K, D>`, `EventCopy<E, C>` (`summary`, `detail`; since S3 the optional `who`, `value`, `empty` the history panel's rows read), `EventSound<E, R>`, `lastEventId`, `newEvents` (briscola-sound-history.md §3.5): the event stream sound and history read alike |
...
| `web/shared/ui/history.ts` | ui (DOM) | `historyRowHtml`, `historyHtml`, `historyKey`, `paintHistory`, `scrollHistoryToEnd` (briscola-sound-history.md §6, PR S3): the other output of the same event, one `<details class="history-row">` per event over the game's `EventCopy`, keyed on the last event id through `ui/keyed.ts`; `HISTORY_IDS` in `ui/ids.ts` names the sheet, `styles/shell.css` styles the rows |
