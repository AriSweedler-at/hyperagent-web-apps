...
C1 split gin's flat `App` into backgammon's two records (`web/games/gin-rummy/src/ui/state.ts` `Shell`/`Table`, `SHELL_INTENT_TYPES`, `reduce = isShellIntent ? shellIntent : tableIntent`) without changing a reducer body (its proof normalises `app.shell.x`/`app.table.x` and `withShell`/`withTable` back to the flat spreads and diffs every `case` against main). The two games agree the six points the config above assumes, so C2 can delegate:
...
3. **`table.cleared(table)` is the one hook a hand's start, leave and loss call.** Gin's `HAND_CLEARED` (`draw`, `picture`, `human`, `drag`) plus the pass-and-play marks (`selectedCard`, `meldChooser`, `revealed` on the shell, `curtain`) at `startLocal`, `handoff`, `leaveFinish`, `host/deal` and `guest/lost`; backgammon's `tableCleared` (everything but `curtainMode`) at the same sites. The shared flows also reset table state on every new view (gin: `selectedCard: null` in `broadcast`/`localBroadcast`/a `state` frame and `resultDismissed: false` on every applied action; backgammon: `selected`/`picked`/`pending: null` in `broadcast`), which `cleared` is not: C2 gives them a second hook (`table.newView`) rather than widening `cleared`.
...
- `shell-home.spec.ts` replaces `gin-home.spec.ts` (44) and `backgammon-local.spec.ts`'s two home tests: the hover walk, the long press (650 ms) opening `#playSubmenu.force-open`, `toHaveText(SHELL[game].tabs)`, the mode switch, the persisted `${storagePrefix}homeTab`/`playMode` keys, `localFields`. Page-only: add to `playwright.config.ts` PAGE_ONLY_SPECS.
