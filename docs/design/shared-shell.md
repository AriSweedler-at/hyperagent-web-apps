...
The figures dated `3b8fc8b` were re-measured there (dry-round-2.md G4) with `wc -l`; a stylesheet's rules are `grep -c '{'` less its `@media` blocks, and a theme's shell residue is its rules whose selectors begin with one of the shell names above.
...
| Table reducer slice (`Table`, `initialTable`, taps, tray, dice, drag, R14 beat, `cuesBetween`/`hitToastsBetween`/`newMovesBetween`, `sandbox/load`, rematch, `tableIntent`) | ~460 at this audit; 679 at `3b8fc8b` (`ui/state.ts` 1,032 lines, `export type Table` through the end of `tableIntent`, lines 216-894, with bg-table-ux's roll modal and tumble and the position/load flow routed through the shell in G2) | Interaction memory of a board game |
