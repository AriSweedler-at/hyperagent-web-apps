...
9. Not shared, on purpose, at this audit: the static shell markup (the DOM-parity oracle snapshots gin's `#homeScreen`; since spelled once as partials by dry-round-2.md F2, #98, each committed page byte-identical), the shell CSS (same 40 selectors, different values; deferred to the Fidice restyle as P13, since shared token-only by dry-round-2.md F1, #103), the engines, the table slices, the e2e table drivers.
10. Every move is protected by suites that already exist and run unchanged against thin wrappers: gin's 87 state tests (51 shell), backgammon's 51 (39 shell), the byte-pinning parity oracles (gin wire, gin storage, gin sessions, gin DOM), backgammon's wire goldens, the computed-style goldens (2 games x 2 viewports), the 120 gin story PNGs (30 of the catalogue's 31 stories x 2 viewports x 2 platforms; `undo-back-to-draw` has none), and the e2e shell specs.
...
| `index.html` shell markup | 146 of 375 (53 ids) | 197 of 597 (43 ids) | 25 (bare `#app`) | 53 ids common (`SHELL_IDS` in `web/shared/ui/ids.ts`, asserted on both pages; 36 counted at this audit, before the waiting rooms, the curtain and the sheets were listed); 32 identical lines (0.066, formatting differs); copy differs throughout | incidental at this audit: keep per game, share the id contract; since dry-round-2.md F2 (#98) the partials render both pages |
| `theme.css` shell rules | 84 lines / 58 rules at this audit (literals: 14px, #fef3c7, `var(--card-2)`); at `3b8fc8b` the shell's rules are `web/shared/styles/shell.css` (409 lines / 61 rules, token-only) and gin's theme keeps 29 rules under the shell selectors (its type scale, the `.card-box` hairline, the `.sheet` border) | 575 lines / 90 rules at this audit (tokens: `var(--radius)`, `var(--bg)`, `var(--accent-dark)`); at `3b8fc8b` its theme keeps 45 rules under the shell selectors and redeclares 44 custom properties on `:root` | 351 lines, own vocabulary (`.panel`, `nav.tabs`, `.opt`); links no shell.css before its restyle | 18 identical lines; the same ~40 selectors (`.tabbar .tab-* .mode-* .card-box .btn* .overlay .sheet* #toast .code-input .room-code .pulse .rules-list`) agree on layout, disagree on colour/radius/font/shadow | shared token-only (dry-round-2.md F1, #103); one type scale is G5, with the Fidice restyle |
...
The figures dated `3b8fc8b` were re-measured there (dry-round-2.md G4) with `wc -l`; a stylesheet's rules are its `{` lines less the `@media` and `@keyframes` lines (the keyframe stops count), and a theme's shell residue is its rules whose selector line names one of the shell selectors above.
...
| Table reducer slice (`card/tap`, `stock/tap`, `discard/tap`, `meld/*`, `arrange/*`, `discards/*`, `card/drag*`, `card/press`, `canDropDiscard`, `fitsMeld`, `rendered` with the cue machine and ghost draw stage, `refuse`) | ~626 at this audit; 604 at `3b8fc8b` (`ui/state.ts` 930 lines, `export type Table` through the end of `tableIntent`, lines 202-805, the shell reducer gone to `web/shared/ui/shell.ts` in C2) | Interaction memory of a card game |
...
| Table reducer slice (`Table`, `initialTable`, taps, tray, dice, drag, R14 beat, `cuesBetween`/`hitToastsBetween`/`newMovesBetween`, `sandbox/load`, rematch, `tableIntent`) | ~460 at this audit; 678 at `3b8fc8b` (`ui/state.ts` 1,032 lines, `export type Table` through the end of `tableIntent`, lines 216-893, with bg-table-ux's roll modal and tumble and the position/load flow routed through the shell in G2) | Interaction memory of a board game |
...
web/shared/
  ui/                         new zone: may import shared/lib, shared/edge/dom.ts, shared/edge/clock.ts; never a game
    ...
    ids.ts                    SHELL_IDS (the 53 common ids) + the page test that asserts each game's markup carries them
...
web/games/<g>/src/
  ...
  main.ts                     bootShell(config) + the game's hook extras (bg ~60-70, gin ~150-170; landed in C3 at 67 and 241, gin's carrying the Score Counter's deps and the card packs' hook members)
```
...
As landed (S3 of docs/design/briscola-sound-history.md, `web/shared/ui/history.ts`): the history sheet's painter is shared before any game's rows are. `historyHtml(events, copy, ctx)` renders one `<details class="history-row" data-kind data-id [data-seat] [data-value]>` per event of a stream (`web/shared/lib/events.ts`), newest last, its `<summary>` led by the copy's `who` span and its `<dl class="history-detail">` the copy's detail pairs (none for a kind the copy gives no rows, so the row is the summary alone); `paintHistory(doc, listId, events, copy, ctx)` keys the list through `ensureKeyed` on the last event id (`historyKey`), so a row the player opened survives every repaint until a new event arrives, and `scrollHistoryToEnd(doc, listId)` brings the newest row into view as the sheet opens. `EventCopy` grew three optional members for the chrome (`who`, `value`, `empty`); `HISTORY_IDS` in `ids.ts` names the sheet's four ids, all already in `SHELL_IDS` (backgammon's `menuHistoryBtn` is its menu sheet's, which gin has no twin of, so it stays a backgammon id). The rows are styled once in `web/shared/styles/shell.css` (F1 having landed first), every selector needing the details element so backgammon's `div.history-row` keeps its theme's look; `data-value` is the shell's weight word (`big`, `huge`, the theme's `--emphasis` and nothing louder). Briscola adopts the panel first; gin and backgammon keep their own `historyHtml` until a later PR turns their rows into `<details>` with a one-line detail.
...
no game; `bootShell` dispatches `resume/auto` after the invite link, which reopens a waiting room
stamped within `WAITING_RESUME_MS` (30 minutes) with no tap and leaves every other offer to
`#resumeBtn`; and `join/link` with the code of the room this device hosts resumes hosting instead
of joining. The same file's §4 records why claim-host is feasible in the lobby and not mid-game.

As landed (C3, `bootShell` in the same file): the ~290 lines both main.ts files spelled around the three helpers moved verbatim (the PR's diff proof normalises the old backgammon boot block and the shared body and diffs them: what differs is the `cfg.` substitution, the injected page objects, the four hook calls and the two spreads), with the config shaped as follows where it amends the sketch above:

1. **Three type parameters, not five.** `bootShell<G, App, Ex>`: `G` the game's `ShellTypes` bag with `Store` fixed to the edge's (`BootTypes`), so `Intent<G>`, `Effect<G>`, `HostFrameOf<G>`, `GuestFrameOf<G>`, `HomeSnapshot<G>`, `Cue<G>`, `TimerId<G>` and the contexts all derive from it; `App` what the boot reads of an App (`BootApp<G>`: `shell.soundFont` and `shell.view`), which a game's `App` satisfies and a three-field test App does too, so boot.test.ts boots a FAKE game without a `ShellState`; `Ex` the game's own effect adapters beside `ShellEffectDeps<G>` (gin `Pick<EffectDeps, 'scorer' | 'copy'>`, backgammon `{}`), spread after the shell's in `deps` so `runEffect` sees its own type.
2. **The page's objects are one group, `cfg.page`**: `doc` (`BootDocumentLike`, the binders' `PageLike` plus `visibilityState`), `win` (`BootWindowLike`: the invite readers' `location`/`history`, `origin`, `confirm`, `scrollTo`, `fetch`, the optional `__rng` and the two Audio constructors), `nav`, `store` and `clock`. main.ts passes `document`, `window`, `navigator`, `browserStore()` and `realClock`, so the boot touches no global but `Math.random` (`RNG_ALLOWED`) and `console.error`, and the test passes fakes. The `navigator` is widened to fx.ts's `NavigatorLike` once, in the boot, where both main.ts files widened it.
3. **The sketch's `game: ShellGame` is three small groups**: `game` (`hook`, the window property; `title`, the share sheet's; `debug`, the PeerJS level: tools/games.ts REGISTRY's three fields, `debug` staying per game for `expectPeerOptions`), `sound` (`enabled(store)`, the game's `soundEnabled`, and `fontKey`, its `STORAGE_KEYS.soundFont`, for the boot-time guard and the console hook's refusal), `net` (`Host`, `Guest`, `isGuestFrame`: the game's wrappers and its protocol's side test). `browserNetDeps({ search: win.location.search, debug })` is built inside the boot.
4. **`reducer`, `paint`, `fx`, `legal`.** `reducer` carries the six functions the boot calls (`initialApp`, `reduce`, `runEffect`, `readHome`, `hostContextOf`, `guestContextOf`); `paint` the painters and binders (`paint`, `bindAll`, `paintSound`, an optional `toastMarks` for backgammon's `hit`, and `fillName`/`fillP2Name`/`setCode`, the game's because gin's `fillNameInputs` fills the Score Counter's inputs too; gin's `paint` closes over `slotHandView`); `fx` is the game's `createFx`; `legal` the engine's `legalActions` for the hook.
5. **Hooks, not `extraEffects`/`hookExtras`.** `cfg.deps: Ex` is a plain object (gin's `scorer.resume` reads `window.__scorer` lazily, as before); `cfg.hooks.home(store)` runs before every home read (gin's card back guard), `render(ctx)` before the binders (gin: the Score Counter's screen and the rules, About and sandbox markup), `bind(ctx)` after them (gin's `scorer.bind()`), `hook(ctx)` the game's own members spread after the shared ones. The shared members are `app` (a getter), `dispatch`, `render`, `showScreen`, `initHome`, `fx`, `legal`, `soundFont`, `soundFontName`; `act` is each game's (its intent is the game's, not a shell intent), as are gin's `setHomeTab`, `setPlayMode`, `layoffs`, `sandbox`, `sandboxMap`, `cardPack`, `cardPackName`, `cardBack` and backgammon's `view`, `setup`. `BootCtx` (`dispatch`, `app()`, `toast`, `fx`, `store`, `now`, `rng`, `homeSnapshot`) is what every hook gets and what `bootShell` returns.
6. **The order is the old one**: the store guards and `homeSnapshot`, rng, wake lock, audio, timers, toaster, cue player, net deps, `dispatch`, the session events, `deps`; then `hooks.render`, `bindAll`, `bindJargon`, `hooks.bind`, `paintSound`, the three warm-on-gesture listeners, `visibilitychange`, the hook on the window, `home/init`, `applyInviteLink`, the `#rule-<id>` dispatch. Gin's `migrateCardBack` stays in main.ts right after `browserStore()`, before the boot, where #92 put it; the stories early return stays first.
7. **The sample fetch** is `fetchArrayBuffer(win)` over `win.fetch`, exported beside the boot so boot.test.ts covers the seam no shipped font plays yet.

boot.test.ts boots the FAKE game over `fakePage`, a recorded window and navigator, a Map store and the fake clock, and pins the boot order (an `order` trace the fakes write to), the no-repaint rule and the effects-then-paint order, every adapter's reach (the toast with the game's marks, `confirm`, `scrollTop`, the timers on the clock, the toggle's repaint, cues in the App's font, the buzz, the share chain to the clipboard, the wake lock, the rule reveal, the three input writes), the sessions' construction over the browser net deps with the App read back live and `send` routed by side, the hook's members and the sound-font hook, the stored-font guard after the game's own, the warm-on-gesture and visibility listeners, and the two links. `web/shared/edge/**` stays above its row (tools/ci/suites.ts records the re-measure).
...
The shell's N-seat surface (landed with the briscola program's PR-5, `briscola-online-seats`; the as-landed shape is n-seat-sessions.md §6.12): every §7 item is an optional extension a two-seat game never sees. `cfg.seats?: { min, max, fixed? }` turns it on (`fixed`: Start and the deal wait for the room's capacity); with it `cfg.opts.capacity?(opts)` names the room's seats (its `max` without one), `ShellState` gains `seats: SeatState[]` (guest seats 1..N−1, `capacity − 1` of them from `startHost` on), `mySeat`, `localNames` and `localSeats`, with `oppName`/`oppConnected` written as `seats[0]`'s on every seat change (`withSeats`) while a two-seat game's flows keep reading the two legacy fields (`seatsOf`), so gin's and backgammon's reducers, painters, `hostContextOf` pins and e2e hooks read what they read. The host's `send` effect carries `seat` only for an N-seat game (`broadcast` is one `send` per connected seat, each `viewFor(game, s)`), `startHost` carries `capacity`, `copy.waiting(capacity)` and, for a resumed or handed-off room, the seats' `names` (seeding the session's rejoin keys, so guests come back to their own seats in any order) only then, the lobby goes round mid-game on every seat lost or back (a guest's table stays the host's), `host/frame` and `host/guestGone` take an optional `seat` (1 when absent) from `sessionEvents(deps, { seats: true })`, which `bootShell` selects through `cfg.net.seats`; `engine.create` takes `PlayersOf<G>` (the pair for a bag with no `Seat`, the list for one with), `engine.renameGuest(game, name, seat)`, `frames.lobby(hostName, opts, seats, you)` and `copy.hostRoom(hostName, opts, seated, capacity)` gain trailing parameters the two-seat builders ignore, and the copy forms `waiting`, `joined`, `seatLeft`, `guestGone`, `roomFull`, `notEnough` are optional with the two-seat strings as their defaults. A guest reads its seat and the table off a welcome or lobby frame structurally (`roomSeatingOf`: `you` and `seats` beside the options, n-seat-sessions.md D3), so no reader enters the config; `paintWaiting` fills `#seatList`/`#guestSeatList` when a page carries them (class-free rows, no CONTRACT.md change); the host save carries `seatNames` only past two seats (`shellSave` decodes it as optional), and a resume reopens the room at the saved capacity with every seat named and disconnected, each guest reseated by name through `codec.joinName`. `bootShell`'s `BootConfig` gains a fourth parameter `HC extends HostContextOf<G>` so a game's `hostContextOf` can return the shell's context plus `seats` for a codec whose welcome lists the table. Proof: gin's and backgammon's `state.test.ts`, wire pins and e2e unchanged; shell.test.ts `FAKE4` (a table of four over the same fake game) covers the join order, the dedupe over every seat, Start gating, per-seat sends and refusals, a seat down mid-game and its rejoin by seat, a lobby leave, the save's `seatNames`, the resume at capacity and the guest's `you`.
...
The second round under the same rules, measured on main after Waves A-B with C2/C3 in flight (what else hoists out of the table UI, the engines, the styles and markup, fidice and the harness, and how each kernel is unit-tested on the fakes), is [docs/design/dry-round-2.md](dry-round-2.md); its Wave D1 (`web/shared/ui/keyed.ts`) landed first; D2 (`bindButtons` and the press-as-function `bindLongPress` in `shellPaint.ts`) is in too; Wave G's G1 (items E6 and E8: `paintConnDot` in `shellPaint.ts`, `homeView` and `shellIntents` in `home.ts`, the two games' `ui/home.ts` down to their own options; `fillNameInputs`/`fillP2NameInput` stay in each `ui/home.ts` as id-list wrappers over the shared `fillInputs`, gin's adding the two Score Counter inputs, because `bootShell`'s config takes them as functions: an id-list config field in boot.ts is a separate row) followed C2 and C3.
...
Deferred, with the reason: **shell CSS** (`web/shared/styles/shell.css`, P13) until the Fidice restyle designs three palettes once (gin must first move its literals onto the eleven tokens; every changed declaration re-records 2 games x 2 viewports of goldens and the 120 story PNGs); the token-only half landed as dry-round-2.md F1 (#103) with one notes-only re-record, and the type scale is G5. **Shell markup partial** (a Vite plugin or boot-time builders for the 53 common ids and the wait/curtain/toast blocks) together with P13, so goldens, CONTRACT.md rows and `page.fake.ts` migrate once; landed as dry-round-2.md F2 (#98) as partials plus each page's `page.ts`, the committed pages byte-identical, so nothing migrated. **Stories boot** (`bootStory`) the day backgammon wants a `?story=` catalogue; landed as dry-round-2.md G3 (#101), `web/shared/ui/stories.ts`. **Dice builder** after the Fidice restyle. **Fidice's client/peerjs** onto the shared sessions: a product decision (§4.6).
...
- `shell-glossary.spec.ts` replaces `gin-glossary.spec.ts` (107) and `backgammon-glossary.spec.ts` (88) (dry-round-2.md I4): the About tap landing on the Rules tab with the rule in view and flashed, the onward link inside a rule, the remembered tab, the `#rule-<id>` deep link at boot, and the rules overlay over a table; the words, the rule ids and the way to the overlay are the `glossary` row of `e2e/fixtures/online-games.ts`. Gin's tabs-fit check moved to `shell-home.spec.ts`, sized by `SHELL[game].tabs.length`. Page-only.
...
| 5 | **The class contract and goldens.** `CONTRACT.md` names an owner per toggled class; `computed-styles` goldens are keyed by screen name and selector; the 120 story PNGs (60 shots x 2 platforms) can only be re-recorded on linux for their `-linux` half | B1, D2, P13 | B1 changes no class name or toggle target; D2 keeps names/order byte-identical or re-records all four goldens in the PR after bg-online; P13 deferred to the Fidice restyle; `expectHandRows` unchanged by A5 |
