...
|---|---|
| `conn`, `live`, `held` | `slots: ReadonlyArray<Slot>`, `Slot = {seat, channel: Cell<Channel \| null>, name: Cell<string \| null>}`, `Channel = {conn, live, slot: Cell<Slot>}`; a vacated seat keeps its last rejoin key |
| `accept`: free → `seat`; silent ≥ HB_MISSED_MS → `replace`; a hold pending → `refuse`; else `hold` with one probe | lowest slot whose channel is null or not open → `seat(conn, slot)`; lowest channel silent ≥ HB_MISSED_MS → `replace(conn, channel)`; a hold pending → `refuse`; else `hold(conn)`: one probe per seated channel, the first `silent` → `replace` (the rest cancelled), `alive` counted to the number probed → `refuse` |
...
| `onError` → `guestGone(iceFailed \| null)`; the channel kept, its watch stopped | the same, per seat |
...
`back = slots.find(s => s !== from && s.name === key && (s.channel === null || s.channel.live.silence() >= HB_MISSED_MS))`;
...
| Coverage row `web/shared/net/**` 95/95/95/94 | `tools/ci/suites.ts` | re-measured: 99.59 / 99.38 / 100 / 99.54 (statements/branches/functions/lines), host.ts at 99.07/98.76/100/98.96 (§6.3 names the two lines) |
...
3. **A channel replaced while it negotiated is closed when it opens.** The free-seat rule ("null or
   not open", the two-seat predicate) lets a later connection take the seat of a channel still
   negotiating (or one whose negotiation failed, whose retry this usually is). The two-seat code
   would still welcome that stale channel when it opened and start its watch, and at its grace
   `gone` would null the slot the new guest now holds and report a loss for it. `opened` now
   checks the channel is still seated, and closes it otherwise (its guest's session sees `close`
   and rejoins). The fake broker opens every channel it accepted, in order, so this path cannot be
   driven in a unit test: host.ts lines 410–411 are the two uncovered lines of the row.
...
8. **The harness**: `world({seats?: boolean})` with two explicit recorders (the design's words) and
   `guests(w, room, n)`; a `Guest` type. `sessions.seats.test.ts` runs 13 scenarios rather than the
   design's 12: the failed negotiation's retry (the "not open" free path at capacity 2) is its own.
...
| 4 | The `web/shared/net/**` row dips as briscola's paths land | the seats suite covers every N path but §6.3's two lines; the row is re-measured in every PR that touches the folder |
