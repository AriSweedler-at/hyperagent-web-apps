...
│   │                            games' token-only shell rules, F1), CONTRACT.md (CSS<->TS class contract + the
│   │                            palettes' token table); every page links the first two, the shell games all three
│   └── games/
...
│                                the partials), ci/ (suites, affected), parity/ (computed-styles, gin-dom-parity),
│                                card-backs, card-packs, linea, legacy/{extract,debundle,record}-*
...
| `web/shared/lib/cards/` | itself | The card packs (docs/design/card-packs.md): `decks.ts` the two deck kinds (`french52` pinned to gin's `makeDeck`, `italian40` with the briscola ids) with `cardIds`, `splitId`, `cardName`; `suits.ts` the four Italian suit symbols as data; `packs.ts` `CARD_PACKS`, `CardPack` (one back, faces per deck kind as `glyph`, a `glyph` with a `relabel` onto another deck kind, `files` or `sprite`, attribution), `packsFor(kind)`, `isCardPackFor`, `DEFAULT_CARD_PACKS`, `badCardPackMsg(key, kind, value)`; `packs/<name>.ts` the packs as data (gin's four backs under gin's names; `linea`, generated by tools/linea.ts; `napoletane`, cut from the owner's sheet by tools/card-packs.ts; `american`, gin's glyphs relabelled for the Italian deck, hand-written); `resolve.ts` `resolveFace` (the pack's picture or the glyph, card by card; a relabelled glyph keeps the card's id), `relabelledId`, `resolveBack`, `resolveAspect`, `attributionLine`. |
| `web/shared/lib/lang/` | itself | The language packs (docs/design/language-packs.md): what a card is called, the sound-font pattern a third time. `packs.ts` `LANGUAGE_PACKS` (`it`, `en`, `en-plates`), `LanguagePack` (a word per suit id and rank id per deck kind, and the word that joins them), `langByName`, `isLanguagePack`, `resolveLang(value, fallback)`, `badLanguageMsg(key, value)`, `cardName(pack, kind, id)` ("re di denari", "king of coins", "ace of spades"; a pack missing the kind or a word falls back to the deck's own language card by card); `packs/<name>.ts` the packs as data (`en-plates` is `en` with the owner's word for denari). |
| `web/shared/ui/cardFace.ts` | shared/lib | Lint-pure like `ids.ts`: `faceHtml(spec, { extra })` (a `french52` glyph is gin's `cardHtml` byte for byte, and the `american` pack's relabelled glyph is that with the Italian id on `data-card`; an `italian40` glyph uses the shared sprite; a `files`/`sprite` face paints the picture as the box's background from the pack's relative URLs), `backHtml`, `backImageCss`, `SUIT_SPRITE_SVG`. |
...
| `tools/shell-markup.ts` | `web/shared/markup`, each shell game's `page.ts`, Prettier | Composes the committed `web/games/<g>/index.html` of every shell game in `PAGES` from the partials and the page's `ShellPage` (`--write`; without it, exit 1 on a drift), formatted with the repo's Prettier config where `.prettierignore` leaves the page to it. `test/dist/shell-markup.test.ts` pins the committed bytes to the render, so the tool runs after every edit to a partial or a `page.ts` (docs/design/dry-round-2.md G2, Wave F row F2, #98; README "Add a game" step 1). |
| `test/shared` | shared/lib, the engine under test | Test code, so no coverage row (docs/design/dry-round-2.md D6, #86): `replay.ts`, the seeded replay driver both engines' nightly replays run (`driveGame` to `done` under a step cap, dice from `mulberry32(seed)` and picks from `mulberry32(seed * 7919)`, the policy once per step before apply; `seeds`, `shard`, `replayScale` over the env knob, `byteStable`, `roundTrips`), and `engine-helpers.ts` (`PLAYERS`, `now`/`NOW`/`epoch`, `viaJson`, `failureOf`, `must`, `countingRng`, `runIntents`). Both are proved on the coin game under `web/shared/example/coin` before either real engine runs them. |
...
`lang(name)` / `langName()` a language pack under `briscola_lang` (the captions, the card tip and
the card view name the cards in it: docs/design/language-packs.md):
