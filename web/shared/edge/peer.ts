// The peer-level plumbing every game's net/ layer shares (docs/ARCHITECTURE.md "Module boundaries":
// net/ may reach only the transport, clock and peer edges), first ported for gin (docs/MIGRATION.md
// step 12) from the legacy multiplayer UI (legacy/gin-rummy/index.html, `whenTransportReady`,
// `peerWatchdog`, `keepPeerAlive`, `announcePath`, `describePeerError`, `relayHint`) over the shared
// Transport (web/shared/edge/transport.ts, the one importer of `peerjs`). What the legacy did with
// `window.Peer` and `window.HyperIce` it does with two injected dependencies: a Transport factory
// that takes the ICE result (a game's main.ts hands it `realTransport`, which honours the `?peer=`
...
// a game's net/ may reach only the transport, clock and peer edges (docs/ARCHITECTURE.md "Module
// boundaries"), so nothing it imports may pull ice.ts in; web/shared/edge/ice.ts's `Ice` satisfies
// `IceLoader` structurally.
