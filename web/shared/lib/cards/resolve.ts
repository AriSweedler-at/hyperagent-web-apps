...
// wrong, the picture is simply the drawn one. The strings the renderer needs (URLs, the alt text,
// the sprite's position) are computed here so web/shared/ui/cardFace.ts only templates them.
import { DECKS, splitId, type DeckKind, type RankSpec, type SuitSpec } from './decks.ts';
import {
  FACE_WIDTH,
  type Attribution,
  type Back,
...
export const resolveFace = (pack: CardPack, kind: DeckKind, id: string): FaceSpec | null => {
  ...
  if (faces === undefined || faces.kind === 'glyph') return glyph;
 ...
 * pictures, else the deck kind's nominal.
 */
export const resolveAspect = (pack: CardPack, kind: DeckKind): number => {
  const faces = pack.decks[kind];
  return faces === undefined || faces.kind === 'glyph' ? DECKS[kind].aspect : faces.aspect;
};
