...
// sound and vibration edges (web/shared/edge/{fx,sound}.ts), with the event table from
// ui/sound.ts and the `ginRummy_sound` preference through storage.ts (docs/MIGRATION.md step 12).
// Since docs/design/sound-fonts.md §5 an event names a cue, not notes: `play` resolves the cue in
// the font the caller passes (the App's `soundFont`, so the reducer stays the source of truth)
// and hands the sound to the edge player. `enabled` lives in the audio cues; a buzz is skipped
// when disabled, as the legacy `buzz` checked `this.enabled`. `toggle` flips and persists the
// preference, warms the context and taps when turning on, then tells the page to repaint
// `#soundBtn` (`renderToggle`, a paint in phase 2). main.ts constructs the real deps; fx.test.ts
// records fakes.
