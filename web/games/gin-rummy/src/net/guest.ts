// The guest's side of a room (docs/MIGRATION.md step 12): `startGuest` and its `tryJoin` from the
// legacy multiplayer UI (legacy/gin-rummy/index.html), frame for frame and timer for timer, over
// the shared Transport. The session owns the Peer, the current channel and every retry: up to
// MAX_JOIN_TRIES connects JOIN_RETRY_MS apart while the host is not registered, a rejoin
// REJOIN_MS after the channel closes, the STALL_MS "answered but can't reach each other" note, and
// the WATCHDOG_MS warning when the broker itself does not answer. Everything it did to the page
// goes through `GuestEvents` and everything it read off `app` comes back through `read()`, so
// net/guest.test.ts drives it over transport.fake.ts and clock.fake.ts. Inbound frames pass
// protocol.ts's decoder; a refused frame is dropped (the legacy ignored non-objects and unknown
// tags). Two legacy traits are kept on purpose: `tryJoin` runs on every Peer `open`, so a broker
// reconnect opens a second channel beside a live one, and a channel that stopped being current
// still reports its frames.
...
export class GuestSession {
  ...
  private tryJoin(peer: PeerHandle, ice: IceResult | null): void {
    ...
    const conn = peer.connect(peerIdFor('gin-rummy', opts.code));
    this.conn = conn;
    let opened = false;
    let failed = false;
    deps.clock.setTimeout(() => {
