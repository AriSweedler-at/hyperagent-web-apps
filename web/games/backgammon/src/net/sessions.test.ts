// The host and guest sessions over the fake broker and the fake clock (design.md §5.3): gin's
// sessions.test.ts in shape and schedule, since the sessions are gin's copies (every status and
// toast, the frames sent, the netAttempt ticket, and the retry schedules: 10 s watchdog, 40 x 3 s
// joins, 1.5 s rejoin, 12 s stall note, 1.5 s path toast, 400 ms then min(15000, 1500 * tries)
// broker reconnects, 1.5 s busy-code retry, 300 ms full close), with this game's differences
// pinned: the `sheshbesh-` peer id and the `welcome`/`lobby` bytes carrying `matchLength` and
// `variant` in place of gin's `target`.
...
describe('HostSession', () => {
  ...
  test('a guest connects: welcome on open, decoded frames reported, refused frames dropped, send() only while open', () => {
    const w = world();
    const ctx = cell(hostCtx({ matchLength: 7, variant: 'backgammon' }));
  ...
  test('the guest leaving: guestGone(null) for the current channel only; after a rejoin the stale one is ignored', () => {
    ...
    w.broker.flush();
    expect(guest.received.at(-1)).toEqual({
      t: 'welcome',
      hostName: 'Ann',
      matchLength: 5,
      variant: 'portes',
...
const hostParty = (w: World, matchLength = 5): Party => {
  const host = party(w, ROOM);
  host.peer.on('connection', (c) => {
    c.onOpen(() => {
      c.send({ t: 'welcome', hostName: 'Ann', matchLength, variant: 'portes' });
    });
    c.onMessage((data) => {
      if (typeof data === 'object' && data !== null && (data as { t?: unknown }).t === 'join')
        c.send({ t: 'lobby', hostName: 'Ann', matchLength, variant: 'portes' });
    });
  });
  return host;
};
...
describe('GuestSession', () => {
  ...
  test('joins: found-the-service status, join frame with the name, connected status, persist, path toast', async () => {
    ...
    expect(w.log).toEqual([
      ...
      ['persist'],
      ['frame', { t: 'welcome', hostName: 'Ann', matchLength: 3, variant: 'portes' }],
      ['frame', { t: 'lobby', hostName: 'Ann', matchLength: 3, variant: 'portes' }],
    ]);
  ...
  test('a host that appears during the retries is joined, and the try counter resets on open', () => {
    ...
    expect(w.log.at(-1)).toEqual([
      'frame',
      { t: 'lobby', hostName: 'Ann', matchLength: 5, variant: 'portes' },
    ]);
  });
