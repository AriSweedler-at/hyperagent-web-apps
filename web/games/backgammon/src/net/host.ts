// The host's side of a room (design.md §5.3): gin's net/host.ts (`startHost` and the `peer`/`conn`
// handlers of the legacy multiplayer UI, frame for frame and timer for timer, over the shared
// Transport) with exactly three edits: the peer id is `peerIdFor('backgammon', code)`, the frames
// come from this game's protocol.ts, and `HostContext` carries `matchLength` and `variant` in
// place of gin's `target`, which is what `welcome` tells the guest. Every `*_MS` constant and
// message string is gin's verbatim, so PR-E (P5) can collapse both files onto one shared session
// with injected codecs. The session owns the Peer, the one guest channel and the timers;
// everything it did to the page goes through `HostEvents` (status text, toasts, the wake lock,
// persisting) and everything it read off the `app` object comes back through `read()`, so
// net/sessions.test.ts drives it over transport.fake.ts and clock.fake.ts. Frames cross the trust
// boundary in protocol.ts: an inbound frame the decoder refuses is dropped.
//
...
export type HostContext = Readonly<{
  ...
  code: string | null;
  myName: string;
  /** The match the room plays: what `welcome` tells the guest (gin's `target` twin). */
  matchLength: number;
...
export class HostSession {
  ...
  private open(ice: IceResult | null): void {
    ...
    const { events } = deps;
    const peer = deps.transportFor(ice).open(peerIdFor('backgammon', opts.code));
    this.peer = peer;
    events.holdWakeLock();
